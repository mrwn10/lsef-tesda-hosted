...
def admin_profile():
    ...
    try:
        ...
        admin_data['date_of_birth'] = admin_data['date_of_birth'].strftime('%Y-%m-%d') if admin_data['date_of_birth'] else None
        admin_data['date_registered'] = admin_data['date_registered'].strftime('%Y-%m-%d %H:%M:%S') if admin_data['date_registered'] else None

        admin_data['profile_picture_url'] = url_for(
            'static', 
            filename=f"uploads/profile_pictures/{admin_data['profile_picture']}", 
            _external=True
        ) if admin_data['profile_picture'] else None

        return jsonify({'success': True, 'admin': admin_data})

    except Exception as e:
        current_app.logger.error(f"Error fetching admin profile: {str(e)}")
